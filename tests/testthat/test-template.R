...
#    `functionName` is a function's name in your module (e.g., `lineDensityEvent1`).
# 2. Copy this file to the tests folder (i.e., `D:/GitHub/modules/lineDensity/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("lineDensity")
  path <- list(modulePath = "D:/GitHub/modules",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    lineDensity = list(.saveInitialTime = NA)
  )
  ...
  if (exists("lineDensityEvent1", envir = .GlobalEnv)) {
    simOutput <- lineDensityEvent1(mySim)
  } else {
  ...
  if (exists("lineDensityEvent2", envir = .GlobalEnv)) {
    simOutput <- lineDensityEvent2(mySim)
  } else {
